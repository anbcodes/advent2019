...
module.exports = class IntcodeComputer {
  constructor(opcodes, id, color, hideOutput) {
    ...
    this.color = color || $.e
    this.customOnOutput = () => {}
    this.onInputAsked = () => {console.log('asked')}
    ...
    this.Program.on(9, this.setOffset.bind(this), 1)
  ...
  onOutput({ opcodes, index }, value) {
    console.log(this.color + String(value.value) + $.e);
  ...
  onInput({ opcodes, index }, toWrite) {
    this.onInputAsked(this)
    console.log(`${this.color}Waiting for input: ${$.e}`)
    return new Promise(resolve => {
      let interval = setInterval(() => {
        if (this.stdin.length !== 0) {
          let value = this.stdin.pop()
          console.log(`${this.color}Input (Async): ${value}${$.e}`);
